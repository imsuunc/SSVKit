...
public extension Notification.Name {
    static let reachabilityChanged  = Notification.Name("reachability.changed")
}
...
public class Reachability: NSObject {
    ...
    required init(reachabilityRef: SCNetworkReachability,
        ...
        self.reachabilitySerialQueue = DispatchQueue(label: "com.ssvkit.queue.reachability", qos: queueQoS, target: targetQueue)
...
public extension Reachability {
   ...
   @available(*, deprecated, message: "Please use `connection == .wifi`")
    ...
    func pushNotify(name: String) {
        let notify = { [weak self] in
            guard let self = self else { return }
            (self.connection != .unavailable) ? self.whenReachable?(self) : self.whenUnreachable?(self)
            self.notificationCenter.post(name: Notification.Name(name), object: self)
        }
